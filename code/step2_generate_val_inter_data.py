...
from data_utils import noclip_filter03_all_info


# task: create val data (intermediate)
# this is the data that is still high quality (error is less than 0.3), but not in Quick_start_data
if __name__ == '__main__':
    ...
    all_data = data["RAW_DATA_TRAIN"]
    write_intermediate_path = data["INTERMEDIATE_VAL_DATA"]

    read_df = pd.read_csv(all_data)

    # drop all seqs that are in Quick_Data
    quick_df = pd.read_csv(quick_data)

    df = read_df[~read_df['sequence'].isin(quick_df['sequence'])]
    ...
    print("created intermediate validation data")
